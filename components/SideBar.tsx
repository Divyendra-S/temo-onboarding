...
const MenuItem: React.FC<MenuItemProps> = ({ title, icon, description }) => {
  return (
    <div className="border border-gray-300 border-opacity-80 bg-white hover:bg-gray-50 text-gray-600 cursor-pointer group flex items-center gap-2 w-full pl-2 pr-3 py-2 rounded-md ">
      <div className="w-9 h-9 rounded-md overflow-hidden border bg-gray-100" style={{ flexShrink: 0 }}>
        <img
          alt="Demo Thumbnail"
          loading="lazy"
          width={34}
    ...
    <div className="flex flex-col gap-3 mb-2">
      <div className="flex items-center justify-between group">
        <div className="flex gap-2 items-center w-full">
          <p className="font-semibold text-sm outline-none w-full text-gray-700">{title}</p>
        </div>
      </div>
      <div className="flex flex-col gap-2">
        {items.map((item, index) => (
          <div key={index} className={index === items.length - 1 ? 'mb-6' : ''}>
            <MenuItem title={item.title} icon={item.icon} description={item.description} />
          </div>
        ))}
      </div>
    </div>
...
const SideMenu: React.FC = () => {
  ...
  return (
    <nav className="w-[295px] h-full bg-white relative flex flex-col border-r border-gray-100 overflow-y-auto">
